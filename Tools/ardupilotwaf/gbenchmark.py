...
def configure(cfg):
    ...
    if not my_src_node:
        cfg.end_msg('not found', color='YELLOW')
        return
    cfg.end_msg('yes')
...
class gbenchmark_build(Task.Task):
    ...
    def run(self):
        ...
        my_src_node = bld.srcnode.find_dir('modules/gbenchmark')
        if not my_src_node:
            bld.fatal('Submodule gbenchmark not found, please run configure again')
