...
union pwm_params {
    ...
    char duty_cycle[sizeof("XXX/pwmXXXXX/duty_cycle")];
    char enable[sizeof("XXX/pwmXXXXX/enable")];
    char period[sizeof("XXX/pwmXXXXX/period")];
    char polarity[sizeof("XXX/pwmXXXXX/polarity")];
};
...
void PWM_Sysfs::enable(bool value)
{
    ...
    int r = snprintf(path, sizeof(path), PWM_BASE_PATH "%u/pwm%u/enable",
                     _chip, _channel);
    if (r < 0 || Util::from(hal.util)->write_file(path, "%u", value) < 0) {
...
bool PWM_Sysfs::is_enabled()
{
    ...
    int r = snprintf(path, sizeof(path), PWM_BASE_PATH "%u/pwm%u/enable",
                     _chip, _channel);
    if (r < 0 || Util::from(hal.util)->read_file(path, "%u", &enabled) < 0) {
        hal.console->printf("LinuxPWM_Sysfs: chip=%u channel=%u "
...
void PWM_Sysfs::set_period(uint32_t nsec_period)
{
    ...
    int r = snprintf(path, sizeof(path), PWM_BASE_PATH "%u/pwm%u/period",
                     _chip, _channel);
    if (r < 0 || Util::from(hal.util)->write_file(path, "%u", nsec_period) < 0) {
        hal.console->printf("LinuxPWM_Sysfs: chip=%u channel=%u "
...
uint32_t PWM_Sysfs::get_period()
{
    ...
    int r = snprintf(path, sizeof(path), PWM_BASE_PATH "%u/pwm%u/period",
                     _chip, _channel);
    if (r < 0 || Util::from(hal.util)->read_file(path, "%u", &nsec_period) < 0) {
        hal.console->printf("LinuxPWM_Sysfs: chip=%u channel=%u "
...
void PWM_Sysfs::set_polarity(PWM_Sysfs::Polarity polarity)
{
    ...
    int r = snprintf(path, sizeof(path), PWM_BASE_PATH "%u/pwm%u/polarity",
                     _chip, _channel);
    if (r < 0 || Util::from(hal.util)->write_file(path, "%s", polarity == NORMAL ? "normal" : "inversed") < 0) {
        hal.console->printf("LinuxPWM_Sysfs: chip=%u channel=%u "
...
PWM_Sysfs::Polarity PWM_Sysfs::get_polarity()
{
    ...
    int r = snprintf(path, sizeof(path), PWM_BASE_PATH "%u/pwm%u/polarity",
                     _chip, _channel);
    if (r < 0 || Util::from(hal.util)->read_file(path, "%s", polarity) < 0) {
        hal.console->printf("LinuxPWM_Sysfs: chip=%u channel=%u "
