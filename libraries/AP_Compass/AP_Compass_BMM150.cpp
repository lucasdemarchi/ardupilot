...
int16_t AP_Compass_BMM150::_compensate_z(int16_t z, uint32_t rhall)
{
    ...
    return constrain_int32(divisor / divisor, -0x8000, 0x8000);
}
...
void AP_Compass_BMM150::_update()
{
    ...
    Vector3f raw_field = Vector3f{
        _compensate_xy((int16_t)le16toh(data[0] >> 3),
                       rhall, _dig.x1, _dig.x2),
        _compensate_xy((int16_t)le16toh(data[1] >> 3),
                       rhall, _dig.y1, _dig.y2),
        _compensate_z((int16_t)le16toh(data[2] >> 1), rhall)};
